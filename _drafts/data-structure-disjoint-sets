...
Once we call `unionSets(0,1)` the vector might look like this: `{0,0,2,3,...,numSets-1}`.
...
To continue the example, if we then call `unionSets(0,2)`, the vector might look like this: `{0,0,2,3,...,numSets-1}`.
...
size_t DisjointSets::findSet(size_t element)
{
    ...
    // Optimization: point directly to representative.
    return nodes[element] = findSet(nodes[element]);
}
```

Note that this is tail recursion so the compiler can turn this into a loop.
...
In addition to the applications I mentioned in the introduction, this data structure can be used to answer the question, "given a graph, how many disconnected subgraphs are there?"
Simply iterate each edge and add both vertices to the data structure, then count the number of sets.
To do this, you would need your implementation to track the number of sets, decrementing it each time a union is performed.
